"""
expand_scene_pool.py — expand the qualified-scene pool for the E1-E4
experiment suite beyond scene 102343992's 9 frozen labels
(embodied/experiment_config.py; E1-E4 need >=100 questions per stratum).

Per-scene pipeline, cheapest first:

  1. Reachability pre-flight (embodied.reachability.check_reachability_invariant)
     — pure scene-geometry check under the frozen NavMeshConfig; a failing
     scene is skipped before any generation is spent on it.
  2. Multi-day generation (gen_dataset subprocess) — 4 train days (day 0
     as its own single-day call so its folder carries no _day suffix,
     days 1-3 in one call) + 1 eval day (day 4, own call/folder), matching
     how 102343992's folders were produced. Profile is per scene: a scene
     with existing generation_out/ folders keeps its profile (persona is
     keyed by household_id = scene_id + household_type); a fresh scene
     gets a deterministic hash-of-scene_id pick from HOUSEHOLD_PROFILES,
     stable across resumed runs.
  3. Qualified-label count — embodied.sampling.qualify_labels()'s
     two-property rule, via compute_frozen_labels.compute_qualifying_labels().
  4. M3 state-change stratum (generate_state_stratum) — deterministic
     no-LLM second pass writing <folder>_state siblings; a folder whose
     merged trace fails trace_validate is reported and left unwritten.

State is checkpointed to a JSON file (--state) after every stage of every
scene, so an interrupted run resumes without repeating completed stages
(gen_dataset's --cache-dir separately caches raw LLM responses). A folder
counts as "generated" only if it passes trace_validate, not on file
existence alone.

Generation runs as a subprocess under _GEN_PYTHON (default: this script's
own interpreter; override with DYNAMIC_EQA_GEN_PYTHON).

Usage:
    python -m dynamic_home_eqa.scripts.expand_scene_pool --n 9
    python -m dynamic_home_eqa.scripts.expand_scene_pool --scenes 102344094 102344115
    # Reachability only, no generation (fast triage across many scenes)
    python -m dynamic_home_eqa.scripts.expand_scene_pool --n 30 --skip-generation
"""
...
from dynamic_home_eqa.paths import HSSD_DIR, REPO_ROOT as _DYNAMIC_EQA
...
# Interpreter for the gen_dataset subprocess. The dynamic_eqa env has both
# habitat_sim and vllm, so this script's own interpreter is the default;
# override via DYNAMIC_EQA_GEN_PYTHON if generation must run elsewhere.
_GEN_PYTHON = os.environ.get("DYNAMIC_EQA_GEN_PYTHON", sys.executable)

_ALREADY_DONE = {"102343992"}  # fully generated + frozen; not a candidate

# Verified by hand via qualify_scene.py under the current navmesh config —
# both PASS. Recorded so this script doesn't repeat the check.
...
def _run_gen_dataset(scene_id: str, profile: str, day: int, n_days: int, out_dir: pathlib.Path, cache_dir: str, model: str) -> None:
    cmd = [
        _GEN_PYTHON, "-m", "dynamic_home_eqa.scripts.gen_dataset",
        "--scenes", scene_id,
        "--profile", profile,
        "--day", str(day),
        "--n-days", str(n_days),
    ...
    proc = subprocess.Popen(
        cmd, cwd=str(_DYNAMIC_EQA), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1,
    )
    lines: list[str] = []

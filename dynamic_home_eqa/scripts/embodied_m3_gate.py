...
tod_prior (M2's zero-live-sensing floor) is in this gate's policy set,
location axis only (a state-axis tod_prior variant would need its own
bucketed state kernels and belief-store plumbing; a documented future
extension).

conformal_decay_threshold is NOT in this gate's policy set: even after
calibration fixes (see belief.calibrate_conformal_theta's docstring),
realized coverage missed its 1-alpha target at every wait_hours bucket
except the shortest — a dwell-time covariate shift, with the residual
explanation that the fitted TransitionKernel's exponential-decay model
doesn't match real multi-hour dwell dynamics — a modeling gap no
calibration scheme can patch. See scripts/conformal_coverage_check.py for
the full diagnosis and results/reports/conformal_coverage_finding_full.md for the write-up (an

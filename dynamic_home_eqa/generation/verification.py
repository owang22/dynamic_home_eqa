...
def resolve_conflicts(
    ...
    for round_idx in range(max_rounds):
        ...
        for occ_name, spans in spans_to_fix.items():
            ...
            if idx is None:
                continue
            # Regenerate the whole trace for this occupant with a
            # conflict-aware hint injected into the seed space.
            # Using round_idx in the stage name gives a fresh seed so
            # the cached original is not reused.
            new_trace = generate_activity_trace(
                ...
                cache=cache,
                force=True,  # always regenerate conflicting traces
            )
...
def run_verification_pass(
    ...
    # Final conflict check for metadata

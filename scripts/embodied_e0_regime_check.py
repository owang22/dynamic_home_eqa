...
Re-baselined (navmesh-connectivity phase, D3): this script had the same
sampling and question-construction drift the M1 gate exposed. Two fixes:

  1. Label sampling used "moved at least once in the eval day", the same
     defect class that produced the M1 gate's 80% abstain rate — some
     sampled labels didn't exist yet at patrol_start, some sat on navmesh
     islands the start pose couldn't reach. Labels are now drawn through
     embodied.sampling.qualify_labels, the same rule FROZEN_LABELS uses.
  2. Episode collection duplicated embodied.attribution.rerun_frozen_e0's
     patrol/dock/ask loop with its own now-removed EmbodiedQuestion type
     and pre-dock_and_wait question construction (asked_t taken before the
     wait, not after — the decay calculation the whole sweep depends on
     was anchored to the wrong instant). Delegates to rerun_frozen_e0
     directly instead of re-implementing it, using the post-M1
     MCQQuestion/Answer(Choice|Abstain) scoring it already implements
     correctly — the same harness FROZEN_LABELS and the M1 gate use, so
     this conclusion rests on the same guarantees theirs does rather than
     a parallel, potentially-drifted reimplementation.
...
one episode was tried first and found to compound: dock_and_wait(w) called
...
Usage:
    /path/to/explore-eqa/bin/python3 scripts/embodied_e0_regime_check.py \\
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(_DYNAMIC_EQA / "e0_raw.csv"))

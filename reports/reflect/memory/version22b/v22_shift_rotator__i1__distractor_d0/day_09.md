# Household memory — after day 9 (10 day(s) observed)

## Persona hypotheses (up to 3, probs sum to 1)
1. (p=0.80) A single adult who works a standard daytime job, likely leaves home early and returns in the evening. — The consistent early departure (work_badge) on weekdays and evening meals (plate at sink) reinforce a standard work schedule. Today's early departure (05:04) and evening meal (18:21) align perfectly with this pattern. The midday anomaly from yesterday appears to be a one-off.
2. (p=0.15) A person with a morning routine that includes preparing a hot beverage to take along, possibly a commuter or outdoor worker. — Thermos appeared at counter at 04:49 today, consistent with early morning preparation for a workday. This strengthens the hypothesis that the thermos is used for a hot drink to take on the commute. The hi_vis_vest and work_badge still suggest a job requiring safety gear.
3. (p=0.05) A person who may have a non-standard work schedule (e.g., shift work or early finish) on some days. — Today's events are fully consistent with a standard workday, so the midday anomaly from yesterday is likely an exception. This hypothesis remains possible but less likely given the dominant pattern.

## Selected evidence
- Day 0, 05:14 — thermos at elsewhere (early morning preparation of a drink to take out, suggests a workday commute or early activity)
- Day 2, 05:27 — work_badge at elsewhere (early departure for work, confirms workday routine)
- Day 2, 14:42 — hi_vis_vest at locker_shelf_e1 (return from work with safety vest, suggests a job requiring high-visibility gear)
- Day 3, 13:19 — work_badge, hi_vis_vest, keys, phone at elsewhere (midday departure with all work items, suggests a shift or early finish)
- Day 4, 05:08 — phone, work_badge at entry_hook_e1 (early morning departure preparation, consistent with workday)
- Day 5, 12:25 — phone at sofa_l1 (relaxed weekend activity, no work items present)
- Day 7, 05:09 — work_badge at entry_hook_e1 (early Monday departure, reinforces weekday routine)
- Day 7, 19:07 — plate at sink_k1 (evening meal cleanup, typical after returning from work)
- Day 8, 11:22 — work_badge at entry_hook_e1 (midday presence of work badge at home, unusual for standard workday; could indicate a late start or day off)
- Day 9, 04:49 — thermos at counter_k1 (early morning preparation of a hot drink, consistent with workday routine)
- Day 9, 05:04 — work_badge at elsewhere (early departure for work, typical weekday pattern)
- Day 9, 18:21 — plate at table_d1 (evening meal, typical after returning from work)
- Day 9, 19:06 — fork at sink_k1 (evening cleanup, consistent with standard workday)

## Notes
Today (Wednesday) showed a strong standard workday pattern: early thermos preparation (04:49), early departure (05:04), evening meal (18:21), and cleanup (19:06). The midday anomaly from yesterday (Tuesday) appears to be a one-off. Need to observe if any midday deviations occur on other days. Weekend pattern (no work items, relaxed activities) remains consistent. No evidence of other household members.
...
## Notes
Only two days observed (Monday and Tuesday). Need to see if newspaper appears on weekends (Saturday/Sunday) to distinguish between daily subscription vs. weekday-only. Also check if plate at sink appears at consistent times on other days to confirm lunch routine. Weekend patterns (days 5-6) will be key to refine hypotheses. The reading glasses and remote on Tuesday reinforce a home-based, possibly older resident. The dining chair movement at 21:20 is new; need to see if it repeats at similar times on other days.
...
Model: `gpt-5.6-terra` — one strict-mode call per schema against `https://api.openai.com`, trivial prompt, `max_completion_tokens=64`. Acceptance is judged by the API's up-front schema validation (a 400 names the offending keyword verbatim below); rejected schemas are re-probed after `to_hosted_schema()`.

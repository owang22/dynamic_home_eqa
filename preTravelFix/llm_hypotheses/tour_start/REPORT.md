# Tour start and re-asking — hh_001

> **Provenance (2026-09-13):** this study was named "cold start" when it ran and was renamed
> without re-running. Its bank has the ORIGINAL tour: Monday 00:00 on day 0, before any
> activity, with every object at its declared home — so the tour handed every hypothesis the
> full rest map, which is why "tour start" ≈ 7-day history below. The exporter now defaults
> to a per-seed random tour instant (`tour_start: random`); nothing on this page was produced
> under that setting.

Stop point per the brief: tour start built, three-layer re-asking built, random-slice policy
...
| LLM tour-start named, **fixed set** | 0.620 ± 0.010 | **1.332 ± 0.028** |
| LLM tour-start anonymized, fixed set | 0.620 ± 0.010 | 1.331 ± 0.027 |
| LLM tour-start named, **re-asking** (days 3, 7) | 0.603 ± 0.010 | 1.347 ± 0.030 |
| LLM tour-start anonymized, re-asking | 0.571 ± 0.010 | 1.461 ± 0.029 |
...
**1. The tour start works as well as a week of history did.** Hypotheses written from one
...
- *The revisions reinstated the full rest map.* The tour-start hypotheses wrote **zero** rest
...
**3. Named ≈ anonymized on tour start — but not because names don't matter.** The fixed-set
arms are identical to three decimals; the median total-variation distance between their
predictions is **0.003**. The tour-start hypotheses cover 21–23 of 35 objects, 0–3 by all
...
removed the one place a tour-start hypothesis states something about most objects. The
...
Tour-start elicitation: 544 s / 15,291 tokens (named), 499 s / 14,019 (anonymized) — a third
...
`results/llm_hypotheses/tour_start/hh_001/<arm>/` — `per_question.jsonl.gz` (mixture and
...
`per_object_oracle.json`. Code: `llm_hypotheses/{prompt,revise,run_tour_start,
analyze_tour_start}.py`, `beliefs/llm_hypothesis_mixture.py` (re-asking layer),
...
for tour start (e.g. rest required on day 0, dropped on revision); per-object hypothesis

...
- target_relationship must be one of: {relationships}.
- target_anchor must be a plausible furniture category or room area name.
...
- Vary target_anchor across candidates when multiple objects could plausibly
  ...
  breakfast candidate pool can include the dining table, the counter, and
  the sink, not just the dining table three times over.
...
Respond only with valid JSON matching the provided schema. No commentary.
""".format(relationships=", ".join(sorted(PARTNR_RELATIONSHIPS)))
...
def generate_displacements(
    ...
    Args:
        ...
        location:      The occupant's activity.location for this window (one
                       ...
                       target_anchor is additionally scoped to furniture/
                       regions in *this room only* (rooms.anchors_in_room /
                       region_names_for_room), not the whole-scene census.
        ...
        anchor_inventory: {furniture_category: count} of real anchor-capable
                       ...
                       census). Falls back to the full static furniture-type
                       vocabulary if not provided (e.g. the not-yet-wired
                       WorldGraph path) — but that fallback is unverified
                       against real scene contents, so pass this whenever
                       it's available.
    ...
    from ..rooms import anchors_in_room, region_names_for_room
    ...
    # Room-scoped anchor vocabulary: an object can only be placed where the
    # occupant physically is. Falls back to the whole-scene census when the
    # room-scoped list comes up empty (geometry coverage gaps are documented,
    # not universal — see rooms.anchors_in_room) or when location is unknown.
    ...
    room_region_anchors = region_names_for_room(location, room_inventory) if location else []
    valid_room_anchors = (
        room_region_anchors
        or sorted((room_inventory or {}).keys())
        or ["room"]
    )

    schema = build_displacement_schema(
...
def score_realism_batch(
    ...
    seed = make_seed(household_id, day, f"realism_{activity}_{start:.2f}", occupant_index)
    ...
    def _validate(result: dict) -> dict:
        return {s["candidate_index"]: s["score"] for s in result["scores"]}

    client = _get_client(model, temperature)
    by_index = generate_json(
        client, _REALISM_SYSTEM, user, REALISM_SCHEMA,
        seed=seed, stage="realism", cache=cache, force=force, validate=_validate,
    )

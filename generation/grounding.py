...
Build this before tuning prompts — a low survival rate may be a prompt
problem or a model problem, and you cannot tell which without the number.
...
def ground_proposal_semantic(
    ...
    Checks:
      ...
      3. For in_region: anchor matches a real region in this scene (via
         SceneRegions loaded from HSSD semantic_config.json), with aliases
         for common LLM-proposed room names.
      4. For on/within/next_to: anchor matches a real furniture category
         present in this scene (via anchor_inventory, env.inventory's
         ANCHOR_CATEGORIES census) — without this check, any anchor string
         that passed schema validation was accepted unconditionally, which
         is not a check at all once the schema constrains the vocabulary.
         anchor_inventory=None preserves the old permissive behaviour, for
...
def ground_displacement_batch_semantic(
    ...
    for prop in proposals:
        stats.total += 1
        result = ground_proposal_semantic(prop, inventory, scene_regions, anchor_inventory)
        if result.accepted:
            stats.accepted += 1
            accepted.append(prop)
        else:

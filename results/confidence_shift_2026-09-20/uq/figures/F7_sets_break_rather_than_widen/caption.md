...
Top: how often the truth was inside the conformal set, against the 90% the method promises (dashed). Bottom: how many places the set contained. At the first sick day the guarantee breaks while the set stays the same width — the method's uncertainty does not notice the change, it simply becomes wrong about it.

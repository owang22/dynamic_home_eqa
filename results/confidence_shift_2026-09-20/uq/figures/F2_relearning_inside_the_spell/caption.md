...
- **About the data:** The counts are per method above; long-context ran on fewer households than the counters. The shaded envelope behind the three language memories spans the highest and lowest of THEIR OWN three estimates on each day — it is the spread between those methods, not a pooled standard error, and it is there so the three read as one class against the 3-day timetable.

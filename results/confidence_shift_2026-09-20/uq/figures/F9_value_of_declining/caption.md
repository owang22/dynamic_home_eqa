...
What being allowed to decline is worth, by window: the decision score under the best threshold for that window, minus the score when the method is forced to answer every question. Higher bars mean the method's own confidence carries information worth acting on. At the moment the routine changes, only the survival-time model gains materially.
...
- **About the data:** Score under the best threshold for that window, minus the score when forced to answer every question. Settled figures for context are in F8's numbers: the survival-time model 4.6 against the timetables' 7.7 and 7.6.

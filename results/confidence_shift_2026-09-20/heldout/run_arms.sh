...
for told in told nottold; do
  ...
  (python3 -m baselines.llm_hypotheses.run_tour_start --household hh_s${s}_p2 --arm passive:longleaf:longleaf_named --bank-dir /home/oliver/robot/dynamic_home_eqa/banks/patrol --room-look --endpoint http://127.0.0.1:8300 --model Qwen/Qwen3.8-27B --out-dir $R/heldout/hyp/study --hyp-subdir patrol $extra > $R/heldout/arm_s${s}_${told}.log 2>&1; echo "arm s$s $told exit $? $(date +%H:%M)") &
done
wait
python3 -m baselines.patrol.hyp --bank $R/heldout/banks/hh_s${s}_p2.jsonl --out $R/heldout/hyp --arms longleaf --told told not_told --passive --dry-run > /dev/null 2>&1
python3 - <<PY
import sys, pathlib
sys.argv = ["x"]
...
bank = R / "banks" / "hh_s${s}_p2.jsonl"
header = json.loads(bank.read_text().splitlines()[0])
for told in ("told", "nottold"):
    arm_dir = R / "hyp/study/hh_s${s}_p2__bank0/arms/passive" / f"passive__longleaf__longleaf_named__{told}"
    out = R / "hyp/logs" / f"hh_s${s}_p2_longleaf_{'told' if told == 'told' else 'not_told'}.jsonl"

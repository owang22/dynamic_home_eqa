...
- why: [follow_up] The look at resident_3 revealed they were carrying nothing, directly contradicting the previous belief that items missing from `counter_k1` might be `ON_PERSON`. This necessitates updating the belief to prioritize `sink_k1` or `OUT_OF_HOUSE` as the destination for transient kitchen items when Noa is present but hands-free.

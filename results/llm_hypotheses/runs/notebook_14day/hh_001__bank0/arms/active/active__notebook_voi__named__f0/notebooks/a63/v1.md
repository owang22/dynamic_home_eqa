...
- why: [review] Condensed d08-d09 scratch memory into BELIEFS. Confirmed sink_k1 as the primary accumulation point for dishes/lunchbox and the invariant location for watering_can_mara. Clarified the distinction between counter_k1 (active work) and sink_k1 (cleaning/storage). Added entry_table_e1 as the anchor for wallet/keys/bowl, distinct from the counter. Maintained the 'empty hands' pattern for resident_1 during kitchen phases. Dropped transient details from d07 that were not confirmed in d08-d09, focusing on the stable patterns observed in the most recent looks.

...
function renderPlanning(){
  const host=$("#planning"); if(!EXTRA||!EXTRA.planning){ host.innerHTML="<p>Planning cost table pending.</p>"; return; }
  const md=EXTRA.planning; const lines=md.split("\n").filter(l=>l.startsWith("|")); if(lines.length<3){ host.innerHTML="<p>Planning cost table pending.</p>"; return; }
  ...
  for(const pop of ["household","person","person2x"]){   // the populations with a real DATA[pop] to plug into
    ...
    for(const key of Object.keys(EXTRA.llm_live[pop]||{})){
      ...
      SERIES.push({k:qkey, name:arm.name, v:col, core:true, dash:!arm.complete, progress:arm.progress,
        gloss:"A live LLM-strategy arm" + (arm.complete?" (finished)":" from an overnight run still in progress — accuracy and confidence only cover the days answered so far") + ".",
        how:arm.progress});
      state.on.add(qkey);
    }
  }

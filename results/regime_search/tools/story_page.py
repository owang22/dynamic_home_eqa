...
import json, os, sys
from rerun_floor import VALUES as RF, by_window_text as rf_windows, per_arm_text as rf_arms
...
:root{color-scheme:light;
  --paper:#f7f5f0;--panel:#fcfcfb;--plot:#ffffff;--anno:#141413;--ink:#141413;--ink2:#52514e;--muted:#7a776f;--line:#e3ded4;--grid:#ebe7df;
...
@media (prefers-color-scheme: dark){:root:not([data-theme="light"]){color-scheme:dark;
  --paper:#1a1a19;--panel:#232322;--plot:#121211;--anno:#f2f1ec;--ink:#f2f1ec;--ink2:#c3c2b7;--muted:#9c9a91;--line:#3a3936;--grid:#2e2d2b;
...
.chart-wrap{position:relative;background:var(--plot);border:1px solid var(--line);border-radius:8px;padding:10px 8px 6px}
...
// Short names for the BOUNDARY, not the window: these label a line, not a band.
const STAGE_EDGE = {sick:"sick", return:"back to normal", sick2:"sick again", return2:"back again"};
// Stage boundaries are drawn as dotted vertical rules in the theme's ink, with the label above the plot area.
// Nothing tints or overlays the region where a line and its standard-error band are drawn -- a wash behind a
// narrow band is exactly what made the bands hard to read. Dotted is reserved for annotation on this page, the
// same as the one-in-ten promise and the 90% target, so a dotted mark always means "we drew this", never data.
function stageMarks(stages, x, T, H, B, opt){
  const o = opt || {};
  const labels = o.labels !== false;
  let g = "";
  for(const st of stages){
    if(!STAGE_EDGE[st.name]) continue;          // the lead-up has no boundary of its own; it is where we start
    const px = x(st.a);
    g += `<line x1="${px.toFixed(1)}" x2="${px.toFixed(1)}" y1="${T}" y2="${H-B}" stroke="var(--anno)" `
       + `stroke-width="1.1" stroke-dasharray="2 3" opacity="0.75"/>`;
    if(labels) g += `<text class="stg" x="${px.toFixed(1)}" y="${T-(o.dy||6)}" text-anchor="${o.anchor||"middle"}" `
       + `fill="var(--anno)">${o.short? "d"+st.a : STAGE_EDGE[st.name]}</text>`;
  }
  return g;
}
...
function renderLine(svgId, xhId, dataFn, labelSuffix){
  ...
  g+=`<rect x="${L}" y="${T}" width="${W-Rt-L}" height="${H-T-B}" fill="var(--plot)"/>`;
  g+=stageMarks(stagesOf(reg), x, T, H, B, {dy:7});
...
function renderSmall(){
  ...
  const bands=()=>`<rect x="${L}" y="${T}" width="${W-Rr-L}" height="${H-T-B}" fill="var(--plot)"/>`
    + stageMarks(stagesOf(reg), x, T, H, B, {short:true, dy:4});
  // 1. alarm raster
...
function gapCellsFor(reg, key, source){
  ...
  // Sampling was only ever run for the language arm, so under that source every other method falls back to
  // its OWN stated confidence rather than dropping off the chart and taking the reference lines with it. That
  // puts two different quantities on one axis, which is fine only because it is said: `mixed` is carried
  // through to every legend entry and end label, and the caption states it once. It is not signalled by line
  // style -- dotted means annotation on this page now, and a second style would undo that.
  const G = EXTRA.gap && EXTRA.gap.populations[reg]; if(!G) return null;
  const M = G.methods[key]; if(!M) return null;
  return {cells:M.cells, days:G.days, mixed: source !== "stated"};
}
// "Perpetua* (own confidence — not sampled)" — the one word per entry that stops the axis being misread.
function gapLabel(e){
  return e.s.name + (e.mixed ? " (own confidence \u2014 not sampled)" : "")
       + (e.dash && !e.s.progress ? " (per person)" : "");
}
...
function gapSeries(){
  ...
  return out.filter(e => dailyGap(reg, e.key, mode, source))
            .map(e => ({...e, mixed: e.key !== SAMPLE_GAP_KEY && source !== "stated"}));
}
...
function renderGapLevel(){
  ...
  let g="";
  g+=`<rect x="${L}" y="${T}" width="${W-Rt-L}" height="${H-T-B}" fill="var(--plot)"/>`;
  g+=stageMarks(gapStages(reg), x, T, H, B, {dy:5});
  for(const v of [0,25,50,75,100]) g+=`<line class="grid" x1="${L}" x2="${W-Rt}" y1="${y(v).toFixed(1)}" y2="${y(v).toFixed(1)}"/><text x="${L-8}" y="${(y(v)+4).toFixed(1)}" text-anchor="end">${v}%</text>`;
  ...
  const ends=[];
  for(const {s,D,dash,mixed} of lines){ const col=`var(${s.v})`;
    const pts=[]; D.mean.forEach((m,i)=>{ if(m!=null) pts.push(`${x(i+1).toFixed(1)},${y(m).toFixed(1)}`); });
    ...
    const last=D.mean.map((m,i)=>[m,i]).filter(a=>a[0]!=null).pop(); if(last) ends.push({s,col,yy:y(last[0]),v:last[0],mixed,dash}); }
  ends.sort((a,b)=>a.yy-b.yy); let prev=-99;
  for(const e of ends){ let yy=Math.max(e.yy, prev+14); prev=yy;
    g+=`<circle cx="${(W-Rt).toFixed(1)}" cy="${e.yy.toFixed(1)}" r="3.5" fill="${e.col}"/><text class="lbl" x="${W-Rt+8}" y="${(yy+4).toFixed(1)}" style="fill:var(--ink)">${gapLabel(e)} · ${e.v.toFixed(0)}%</text>`; }
  svg.innerHTML=g;
  const anyMixed = lines.some(l=>l.mixed);
  $("#gapconfnote").textContent = lines.length
    ? (state.gapSource==="sampling"
        ? "How often the ten samples agreed with the answer given, per day"
          + (anyMixed ? " \u2014 for the sampled arm only. The other lines are each method's own stated confidence, marked as such where they are named, because sampling was run for the language memory alone. Compare their SHAPES, not their heights: agreement among ten samples and a self-reported probability are not the same quantity, so one sitting above another here means little. The panel below, which measures each line against its own accuracy, is the fair comparison." : ".")
        : "The confidence each method states, per day.")
...
function renderGap(){
  ...
  let g="";
  g+=`<rect x="${L}" y="${T}" width="${W-Rt-L}" height="${H-T-B}" fill="var(--plot)"/>`;
  g+=stageMarks(gapStages(reg), x, T, H, B, {dy:6});
  ...
  for(const {s,D,dash,mixed} of lines){ const col=`var(${s.v})`;
    ...
    const pts=[]; D.mean.forEach((m,i)=>{ if(m!=null) pts.push(`${x(i+1).toFixed(1)},${y(m).toFixed(1)}`); });
    g+=`<polyline fill="none" stroke="${col}" stroke-width="${dash?1.6:2.2}" stroke-linejoin="round" ${dash?'stroke-dasharray="5 3"':""} points="${pts.join(" ")}"/>`;
    const last=D.mean.map((m,i)=>[m,i]).filter(a=>a[0]!=null).pop(); if(last) ends.push({s,col,yy:y(last[0]),v:last[0],dash,mixed}); }
  ends.sort((a,b)=>a.yy-b.yy); let prev=-99; for(const e of ends){ let yy=Math.max(e.yy, prev+14); prev=yy; g+=`<circle cx="${(W-Rt).toFixed(1)}" cy="${e.yy.toFixed(1)}" r="3.5" fill="${e.col}"/><text class="lbl" x="${W-Rt+8}" y="${(yy+4).toFixed(1)}" style="fill:var(--ink)">${gapLabel(e)} · ${e.v>0?"+":""}${e.v.toFixed(0)}pp${e.s.progress?" "+e.s.progress:""}</text>`; }
  g+=`<line id="gapxh" class="axis" x1="0" x2="0" y1="${T}" y2="${H-B}" style="display:none"/>`;
  svg.innerHTML=g; svg.dataset.geom=JSON.stringify({L,Rt,W,nd});

  const popTxt = POP_LABEL[reg] || reg;
  const srcTxt = source==="sampling"
    ? "how often the sampled arm's 10 answers agree with the one it gave, and each other method's own stated confidence"
    : "the number each method states";
  ...
  const mixedNames = gapSeries().filter(e=>e.mixed).map(e=>e.s.name);
  $("#gapsrcnote").innerHTML =
    (mixedNames.length
      ? `<b>Two quantities on one axis, on purpose.</b> The long-context line is the share of its ten sampled answers `
        + `that agree with the answer it gave. ${mixedNames.join(", ")} ${mixedNames.length===1?"is":"are"} each drawn from `
        + `its own stated confidence, because sampling was only ever run for the language memory. They are kept on the `
        + `chart rather than dropped so the comparison keeps its shape, and every one of them is marked `
        + `\u201cown confidence \u2014 not sampled\u201d where it is named.`
        + (mode==="leadcal" ? " Under the lead-day translation each line is calibrated on its own lead days using its own quantity: the counters on their stated confidence, the language arm on its agreement." : "")
        + "<br>"
      : "")
    + (miss.length ? `<b>No reading at all for:</b> ${miss.join(", ")}.<br>` : "")
  ...
  for(const {s,key,dash,mixed} of series){ const M=G.methods[key]; if(!M||!M.summary) continue; const sm=M.summary; const idx = mode==="raw"?"raw":"leadcal";
    const cell=w=>{ const v=sm[w] && sm[w][idx]; return v? `${v.gap>0?"+":""}${v.gap.toFixed(0)} <span class="muted">±${v.sd.toFixed(0)}</span>` : "–"; };
    th+=`<tr><td><i class="swatch" style="background:var(${s.v})"></i>${gapLabel({s,dash,mixed})}</td><td class="num">${cell("lead")}</td><td class="num">${cell("d14_16")}</td><td class="num">${cell("d24_26")}</td></tr>`; }
...
function renderGapConformal(){
  ...
  const bands=()=>`<rect x="${L}" y="${T}" width="${W-Rr-L}" height="${H-T-B}" fill="var(--plot)"/>`
    + stageMarks(gapStages(reg), x, T, H, B, {short:true, dy:4});
...
const RF_JS = /*RERUN_FLOOR*/null;
const rfWindows = /*RERUN_FLOOR_WINDOWS*/"";
const rfArms = /*RERUN_FLOOR_ARMS*/"";
...
const PANELS = [
  ...
  {id:"D", group:"The LLM memories", title:"Long-context memory on its own", pop:"person",
   cap:"The whole log in every prompt, on all ten households and about ten times the compute per question of the other memories. Its three message arms finished on the night of 22 September, so this is now a matched ten-household comparison rather than the three-household sample it was for most of the day.",
...
function drawPanel(p0){
  ...
  const W=400, H=232, L=32, Rr=10, T=20, B=26;   // T leaves room above the plot for the boundary labels
  ...
  g += `<rect x="${L}" y="${T}" width="${W-Rr-L}" height="${H-T-B}" fill="var(--plot)"/>`;
  const R = DATA[p.pop] || DATA[p.stagesFrom || "person"];
  if(R && R.stages){ const st=[]; let cur=null;
    for(let d=1; d<nd; d++){ const nm=R.stages[String(d)]||"plain";
      if(!cur||cur.name!==nm){ cur={name:nm,a:d,b:d}; st.push(cur); } else cur.b=d; }
    g += stageMarks(st, x, T, H, B, {dy:6});
  }
  ...
  order.forEach(i => { const line = p.lines[i];
    ...
    const from = clipFrom[i] || 1;
    let band="", path="", pen=false;
    for(let j=0;j<S.mean.length;j++){ const d=j+1; if(S.mean[j]==null || d<from){ pen=false; continue; }
    ...
    for(let j=0;j<S.mean.length;j++){ if(S.mean[j]==null || j+1<from){ flush(); continue; } run.push([j+1,S.mean[j],S.se[j]||0]); }
    flush();
  ...
  const clipTxt = Object.keys(clipFrom).map(i => `${p.lines[i].label} from day ${clipFrom[i]}`).join(", ");
  const armNote = useArms
    ? "One memory told different things: one hue, three lightnesses, drawn bottom to top as told twice, told once, "
      + "never told. Each told arm is the SAME run as the one above it until the day it is told, so it is drawn only "
      + "from the day it actually departs \u2014 a line beginning partway through is not missing data, it is that day. "
      + "The departure days are measured from the data here rather than taken from the calendar, so an arm that "
      + "separated earlier than its message would show it: " + (clipTxt || "no arm departs at all") + "."
    : "";
...
function drawChannels(){
  ...
  g += `<rect x="${L}" y="${T}" width="${W-Rr-L}" height="${H-T-B}" fill="var(--plot)"/>`;
  const R=DATA.person;
  if(R&&R.stages){ let cur=null; const STG=[];
    const rect=c=>{ STG.push(c); return ""; };
    ...
    if(cur) rect(cur); g += stageMarks(STG, x, T, H, B, {dy:5});
    if(cur) g+=rect(cur); }
  for(const v of [0,25,50,75,100]) g+=`<line x1="${L}" y1="${y(v).toFixed(1)}" x2="${W-Rr}" y2="${y(v).toFixed(1)}" stroke="var(--line)" stroke-width="1"/><text x="${L-5}" y="${(y(v)+3.5).toFixed(1)}" text-anchor="end" font-size="9" fill="var(--muted)">${v}</text>`;
    ...
    if(c.custom==="perpetua"){
      ...
      return `<section class="claim" id="claim${c.n}">
        ...
        <div class="cbody">
          ...
          <p class="pcap" style="max-width:74ch"><b>What it does differently, in one sentence you can check against the method.</b> A timetable's confidence comes from how <em>regular</em> the past was — how tightly its sightings cluster in a two-hour slot. The survival-time model instead treats each object-and-place as something that persists until a survival time runs out, so its confidence comes from how <em>old</em> its evidence is. When the routine changes, a timetable's confidence does fall — but nowhere near far enough. The timetable that never forgets loses about a sixth of its stated confidence (60.9% to 50.9%) while losing two fifths of its accuracy (82.2% to 51.2%). <b>It is not confident exactly where it is wrong</b> — we checked, and its confidence is if anything slightly LOWER on the objects that actually moved. The defensible statement is that its confidence barely moves relative to what happened to it, so what it claims stops standing in any useful relation to whether it is right.</p>
          <p class="pcap" style="max-width:74ch"><b>The mechanism, stated as what we measured.</b> A method's confidence stops predicting its own correctness exactly at the disruption that breaks that method. In the settled weeks all three are about equally good at knowing when they are right (correlation between stated confidence and being correct: +0.28 to +0.37). At the change, both timetables stop — the one that never forgets falls 0.47 and the three-day one 0.28, and what is left of either cannot be told from zero — while the survival-time model is unchanged at +0.43. On the return the split follows which method breaks there: the timetable that never forgets does not break and regains its footing at once (+0.39), the three-day timetable does break and is still not predicting (+0.11) until a week later. The survival-time model's accuracy breaks at both boundaries as hard as anyone's and its confidence keeps tracking regardless.</p>
...
function renderGist(){
  ...
  if(pv("llm_longcontext_startmsg","d14_16")){
    ...
    items.push(`<b>Long-context memory — the whole log in every prompt — finished on all ten households on the night of 22 September, and it is now the memory on which the whole message argument is measured end to end.</b> It costs roughly ten times the compute of the other memories \u2014 about 10,000 to 12,000 tokens of prompt per question against 1,100 for retrieval and 1,400 to 1,500 for reflection, counted from the no-message arms\u2019 call logs \u2014 which is why its three message arms were the last to finish. All three now cover the same ten households and the same questions, so what follows is a matched comparison rather than the three-household sample it was for most of the day, and every figure in it clears our bar. <b>Told on the first sick day, it adapts at once and stays ahead</b>: ${q("llm_longcontext_startmsg","d14_16","all")} on the first three sick days (cold ${q("llm_longcontext_startmsg","d14_16","cold")}) and ${q("llm_longcontext_startmsg","d17_23","all")} through the rest of the spell. <b>Left standing once the routine reverts, the same sentence costs</b>: ${q("llm_longcontext_startmsg","d24_26","all")} on the first days back and ${q("llm_longcontext_startmsg","d27_31","all")} a week later (cold ${q("llm_longcontext_startmsg","d27_31","cold")}). <b>Retracted on the first day back, the cost is gone</b> \u2014 told at both ends it sits ${q("llm_longcontext_startend","d24_26","all")} against never being told, which does not clear the bar, so the residual is smaller than we can measure rather than shown to be zero; what does clear is the retraction against telling once. That is the argument in three clauses, each with a number behind it from the same memory: it can represent the new regime the moment it is told, it cannot notice on its own that the instruction expired, and taking the instruction back repairs what leaving it standing cost. The other language memories move the same way on the return, which is what made this claim survivable when it rested on three households; it no longer rests on them.`);
  }
...
open(out, "w").write(HTML.replace("/*DATA*/null", json.dumps(data, separators=(",", ":"))).replace("/*EXTRA*/null", json.dumps(extra, separators=(",", ":")))
     .replace("/*RERUN_FLOOR*/null", json.dumps(RF, separators=(",", ":")))
     .replace('/*RERUN_FLOOR_WINDOWS*/""', json.dumps(rf_windows()))
     .replace('/*RERUN_FLOOR_ARMS*/""', json.dumps(rf_arms()))

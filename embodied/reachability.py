...
def _make_sim(scene_id: str, navmesh: NavMeshConfig):
    import habitat_sim
...
def check_reachability_invariant(
    ...
    sim = _make_sim(scene_id, navmesh)
    try:
        pf = sim.pathfinder
        ...
        def _own_island_area(pos) -> float:
        ...
        for room, (x, z) in centroids.items():
            pos3 = (x, 0.1, z)
            if _own_island_area(pos3) < navmesh.min_component_area_m2:
        ...
        for slot, pos in anchors.items():
            if _own_island_area(pos) < navmesh.min_component_area_m2:

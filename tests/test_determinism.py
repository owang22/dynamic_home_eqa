"""
Determinism test: the same (scene_id, profile, seed, day) must produce a
byte-identical generated day. Uses a fake LLM client (schema-conformant,
seed-derived pseudo-random) so this runs fast with no GPU — the property
under test is that the *pipeline's own* seed derivation and state-threading
are deterministic, not that any particular model is; the pipeline's
reproducibility contract (generation/README.md) rests on every call
receiving an explicit seed and nothing downstream introducing incidental
nondeterminism (dict/set iteration order, wall-clock, hashing) into the
Change-log.
"""
from __future__ import annotations

import json
import random

import pytest

from dynamic_home_eqa.generation import llm_client as llm_client_mod
from dynamic_home_eqa.generation.manifest import build_manifest
from dynamic_home_eqa.generation.pipeline import generate_for_scene


def _fake_generate(self, system, user, schema, seed=None):
    # Deliberately keyed only on `seed` (and the schema's own enums), never
    # on call count or wall-clock — a real determinism test would be
    # meaningless if the fake itself carried hidden state across calls.
    rng = random.Random(seed)
    props = schema.get("properties", {})

    if "proposals" in props:
        item_schema = schema["properties"]["proposals"]["items"]
        branches = item_schema.get("oneOf", [item_schema])
        out_proposals = []
        for _ in range(rng.randint(0, 5)):
            branch = rng.choice(branches)
            bp = branch["properties"]
            item = {
                "object_category":     rng.choice(bp["object_category"]["enum"]),
                "target_relationship": rng.choice(bp["target_relationship"]["enum"]),
                "target_anchor":       rng.choice(bp["target_anchor"]["enum"]),
                "reason":              "fake reason",
            }
            if "assumed_from" in bp:
                item["assumed_from"] = rng.choice(["kitchen counter", "unknown", "the table"])
            out_proposals.append(item)
        result = {"proposals": out_proposals}
        if "activity" in props:
            result["activity"] = "fake_activity"
        if "occupant" in props:
            result["occupant"] = "fake_occupant"
        return json.dumps(result)

    if "occupants" in props:
        occ_schema = props["occupants"]["items"]["properties"]
        age_band_enum = occ_schema["age_band"]["enum"]
        names = ["Alex", "Sam", "Jordan"]
        n = rng.randint(1, 3)
        occupants = [{
            "name": names[i], "age_band": rng.choice(age_band_enum), "role": "member",
            "tidiness": round(rng.random(), 2), "typical_wake": round(rng.uniform(5, 9), 1),
            "typical_sleep": round(rng.uniform(20, 25), 1), "habits": "fake habits",
        } for i in range(n)]
        return json.dumps({"occupants": occupants, "household_type": "fake"})

    if "activities" in props:
        locs = [l for l in props["activities"]["items"]["properties"]["location"]["enum"] if l != "away"]
        n_acts = rng.randint(3, 6)
        acts, t = [], round(rng.uniform(5, 8), 1)
        for _ in range(n_acts):
            end = round(t + rng.uniform(0.5, 4.0), 1)
            acts.append({
                "activity": rng.choice(["cooking", "resting", "working", "cleaning"]),
                "location": rng.choice(locs), "start": t, "end": end,
            })
            t = end
        acts.append({"activity": "sleep", "location": "bedroom", "start": t, "end": round((t - 17.5) % 24 or 6.0, 1)})
        return json.dumps({"occupant_name": "unused", "activities": acts})

    if "scores" in props:
        return json.dumps({
            "scores": [{"candidate_index": i, "score": round(rng.random(), 2), "reason": "fake"} for i in range(15)]
        })

    return json.dumps({"conflicts": []})


@pytest.fixture(autouse=True)
def _patch_llm_client(monkeypatch):
    monkeypatch.setattr(llm_client_mod._LLMClient, "generate", _fake_generate)


def _run(scene_id, profile, day, variant):
    result = generate_for_scene(
        scene_id=scene_id, household_type=profile, day=day, variant=variant,
        cache_dir=None, force=True, use_semantic_grounding=True,
    )
    manifest = build_manifest(scene_id, profile, day, result, seed=variant + 1)
    return result, manifest


def test_same_inputs_produce_byte_identical_manifest():
    _, manifest_a = _run("102343992", "family_with_kids", day=0, variant=0)
    _, manifest_b = _run("102343992", "family_with_kids", day=0, variant=0)
    assert json.dumps(manifest_a, sort_keys=True) == json.dumps(manifest_b, sort_keys=True)


def test_same_inputs_produce_byte_identical_generation_result():
    result_a, _ = _run("102344022", "single_retiree", day=1, variant=1)
    result_b, _ = _run("102344022", "single_retiree", day=1, variant=1)
    assert json.dumps(result_a, sort_keys=True) == json.dumps(result_b, sort_keys=True)


def test_different_variant_produces_different_output():
    _, manifest_v0 = _run("102343992", "family_with_kids", day=0, variant=0)
    _, manifest_v1 = _run("102343992", "family_with_kids", day=0, variant=1)
    assert json.dumps(manifest_v0, sort_keys=True) != json.dumps(manifest_v1, sort_keys=True)


def test_different_day_produces_different_activity_trace():
    result_day0, _ = _run("102343992", "family_with_kids", day=0, variant=0)
    result_day1, _ = _run("102343992", "family_with_kids", day=1, variant=0)
    # Persona is day-invariant by design; traces are not.
    assert result_day0["persona"] == result_day1["persona"]
    assert result_day0["traces"] != result_day1["traces"]

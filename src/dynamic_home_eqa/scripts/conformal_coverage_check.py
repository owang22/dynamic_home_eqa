#!/usr/bin/env python3
"""
conformal_coverage_check.py — measures realized coverage of
conformal_decay_threshold's calibrated theta on HELD-OUT data (the eval
day, never used for calibration) against the configured 1-alpha target.
Unit tests on belief.calibrate_conformal_theta only show the code runs on
synthetic data; this script checks whether the guarantee actually holds
on real held-out data.

Split-conformal exchangeability: the 1-alpha coverage guarantee only
holds if calibration (train) and test (eval) data are exchangeable draws
from the same distribution. Day-to-day variation in which objects move,
and profile-to-profile variation in routine, can both break this. This
script measures whether it holds for the given scene's train/eval split —
reported straight either way, never tuned to force a pass.

Acceptance: the target coverage (1-alpha) falls within the Wilson score
interval of OBSERVED held-out coverage given the held-out sample size —
not "observed >= target exactly" (a small sample won't hit the target
exactly even when well-calibrated). If the target falls outside the
interval: report the drift; do not adjust alpha/theta to make it pass.

A single global theta miscalibrates across wait horizons (dwell-time
covariate shift: most natural dwell events are short, so a global
quantile is dominated by short-dwell behavior and degrades monotonically
with wait_hours — diagnose_wait_hours_coverage makes this visible).
The Mondrian (group-conditional) variant calibrates a separate theta per
wait_hours bucket (belief.calibrate_conformal_theta_by_wait,
DecayThresholdConfig.theta_by_wait); verify_bucketed_coverage measures
its per-bucket held-out coverage. Full findings history:
results/reports/conformal_coverage_repair.md and
conformal_coverage_finding_full.md.
"""
...
def main() -> None:
    ...
    out_results_dir = _DYNAMIC_EQA / "results" / "e1e4"

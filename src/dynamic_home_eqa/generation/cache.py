...
class ResponseCache:
    ...
    def get_record(self, seed: int) -> dict | None:
        """Return the full cached record ({seed, prompt, raw[, think]}), or
        None if not cached. Use when provenance fields (the think trace)
        are needed alongside the raw payload."""
        p = self._path(seed)
        if p.exists():

"""validate_charter.py <charter.yaml> -- V1-V5 (structural) + V6a-V6e (anchor).

Writes anchor_report.md next to the charter (or --report PATH); exits nonzero
if any check is FAIL. The bank builder calls this and refuses charters that are
not fully PASS/WARN with status VERIFIED.

Provenance tags ([ATUS] [BEHAV] [HOMER] [HKEEP] [DESIGN]) live in the charter's
...
Check status vocabulary:
  ...
  SKIP       no values carry this tag in this charter

Run:  python -m dynbelief.anchors.validate_charter charters/manual/single_adult_typ_v1.yaml
"""
...
from dynbelief.charters.schema import (
    Charter, load_charter, validate_structural, default_class,
)
...
def scan_tags(path: pathlib.Path, ch: Charter) -> list[TaggedLine]:
...
def check_v6e(ch: Charter, tagged: list[TaggedLine], env: dict) -> Check:
...
def check_v6c(ch: Charter, env: dict, n_days: int, seed: int) -> Check:
...
def check_v6a(ch: Charter, tagged: list[TaggedLine], env: dict) -> Check:
...
def check_v6b(ch: Charter, tagged: list[TaggedLine], env: dict) -> Check:
...
def validate(path: pathlib.Path, n_days: int = 30, seed: int = 0) -> tuple[list[Check], Charter]:
    load_findings: list = []
    ch = load_charter(path, load_findings)
...
def render_report(path: pathlib.Path, ch: Charter, checks: list[Check]) -> str:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("charter", type=pathlib.Path)
    ap.add_argument("--report", type=pathlib.Path, default=None,
                    help="anchor_report.md path (default: alongside charter)")
    ap.add_argument("--days", type=int, default=30)
    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args(argv)

    checks, ch = validate(args.charter, n_days=args.days, seed=args.seed)
    report = render_report(args.charter, ch, checks)
    out = args.report or args.charter.with_name(
        args.charter.stem + ".anchor_report.md")
    out.write_text(report)
    print(report)
    print(f"\n[validate_charter] wrote {out}")

"""Symbolic generator: charter -> N-day receptacle-level movement logs.
...
replays charter worlds unchanged.

Dynamics semantics (deterministic given (charter, seed)):
  ...
  * Jitter: each realized block start = nominal + U[-j, +j] (integer
    ...
    merged (union — covers the spec charter's Fri-night sleep meeting the
  ...
  * Dish-cycle (generator-level, all charters, per the schema comment):
...
from dynbelief.charters.schema import (
    Charter, DAY_IDX, MIN_PER_DAY, default_class,
)
...
def _seed_int(charter: Charter, seed: int) -> int:
    h = hashlib.sha256(f"{charter.household}:{seed}".encode()).hexdigest()
    return int(h[:16], 16)


def realize_schedule(ch: Charter, n_days: int, rng: random.Random) -> list[RealizedBlock]:
...
def simulate(ch: Charter, n_days: int = 30, seed: int = 0):
...
def write_episode(ch: Charter, out_dir: str | pathlib.Path,
    ...
    registry = {
        ...
        "generator": {"kind": "charter_symbolic_v1", "seed": seed,
                      "charter": ch.household, "status": ch.status,

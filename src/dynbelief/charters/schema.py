"""Charter schema v1: loader + structural validator (rules V1-V5).

Spec: charter_schema_v1.yaml (Section A). Semantics decisions made here and
flagged for researcher review:
  ...
  * V3 severity split: nominal overlap of two DIFFERENT activities is FAIL;
    ...
    own charter, so the strict version is reported as WARN, not FAIL.
  ...
  * `class` is an optional placements field (defaults to the object id with
    ...
    class "phone"). Multi-resident charters need per-resident object
...
receptacle id before anything downstream sees the charter. This is the
...
def charter_from_dict(data: dict, findings: Optional[list[Finding]] = None) -> Charter:
    """Parse + V5-normalize a charter dict. Structural checks are separate
...
def load_charter(path: str | pathlib.Path,
                 findings: Optional[list[Finding]] = None) -> Charter:
    data = yaml.safe_load(pathlib.Path(path).read_text())
    return charter_from_dict(data, findings)


def require_verified(charter: Charter, allow_draft: bool = False) -> None:
    """Bank-build gate (hard rule): refuse DRAFT charters unless the caller
    passed the explicit dev-mode override — and even then, shout."""
    if charter.status == "VERIFIED":
        return
    if allow_draft:
        import sys
        print(f"[charters] WARNING: {charter.household} is status={charter.status}; "
              f"proceeding under --allow-draft DEV MODE — results are NOT reportable "
              f"until the charter is human-verified.", file=sys.stderr)
        return
    raise RuntimeError(
        f"charter {charter.household} has status={charter.status}, not VERIFIED. "
...
def validate_structural(ch: Charter) -> list[Finding]:
    out: list[Finding] = []

"""Diagnostic probes for the tour-start study, per household.
...
def reask_probe(household: str, condition: str) -> Dict[str, Any]:
    F = {r["question_id"]: r for r in load(household, f"passive__llm_fixed__tour_{condition}")}
    R = {r["question_id"]: r for r in load(household, f"passive__llm__tour_{condition}")}
    if not F or not R:
        return {}
...
def write_probes(household: str) -> str:
    ...
    for c in conds:
        ...
        L.append("")
    if len(conds) == 2:
        idp = identity_probe(household, "passive__llm_fixed__tour_named", "passive__llm_fixed__tour_anonymized")
    ...
    for c in conds:
        r = reask_probe(household, c.replace("tour_", ""))
        if not r: continue

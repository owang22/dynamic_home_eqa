...
class DiscountedTimetable(TimetableLookup):
    ...
    def discount(self, t: int, factor: float, objects=None) -> None:
        self._discounts.append((t, factor))

    def weight_since(self, t_obs: int, t: int) -> float:
        w = 1.0
        for ta, f in self._discounts:
            if t_obs < ta <= t:
                w *= f
        return w

    def _predict_from_history(self, history: List[Tuple[int, str]], t: int):
        ...
        for ot, rec in pool:
            ...
            counts[rec] = counts.get(rec, 0.0) + age * self.weight_since(ot, t)
...
def run(bank_path: pathlib.Path, agent: str, a, seed: int = 0) -> Tuple[List[dict], dict]:
    ...
    rng = random.Random(f"{seed}:uq:{agent}")
    ...
    K = len(beliefs)
    weights = [1.0 / K] * K
    obj_weights: Dict[str, List[float]] = {}     # --per-object: one weight vector per object
    ...
    conformal = DecayingStepConformal(a.alpha, a.eta0, a.eps, a.eta_min)
    nexcp = WeightedQuantileConformal(a.alpha, None if a.tau_w <= 0 else a.tau_w)
    mart = MixtureMartingale(rng, a.delta, a.min_window, cusum=(a.detector == "cusum"), window=a.window or None)
    fires: List[dict] = []
    ...
    def w_of(object_id: str) -> List[float]:
        return obj_weights.get(object_id, [1.0 / K] * K) if a.per_object else weights
    ...
    def sighting(object_id: str, rec: str, t: int, cls: str) -> None:
        """A positive sighting: update BMA weights from each counter's predictive probability, then remember it."""
        nonlocal weights
        for b in beliefs:
        ...
        if K > 1 and rec not in (ON_PERSON, OUT_OF_HOUSE):
            ...
            for b in beliefs:
                ...
                liks.append(max(d.get(rec, 0.0), 1e-3))
            w = [wk * lk for wk, lk in zip(w_of(object_id), liks)]
            z = sum(w)
            w = [(1 - a.share) * x / z + a.share / K for x in w]
            if a.per_object:
                obj_weights[object_id] = w
            else:
                weights = w
    ...
    for q in questions:
        ...
        owner = owners.get(q.object_id.rsplit("_", 1)[-1].lower())
        ...
        if agent == "ocp":
            ...
            if a.look and len(cset) > 1:
                ...
                scores = {s: 1.0 - rel * dist.get(s, 0.0) for s in spots}
                cset = conformal.set_of(scores)
            rec.update({"set_size": len(cset), "q_t": round(conformal.q, 4), "rel": round(rel, 4)})
        if agent == "nexcp":
        ...
        if agent == "ocp":
            ...
            key = (last_seen.get(q.object_id), truth)
            dup = a.dedupe and seen_q.get(q.object_id) == key
            seen_q[q.object_id] = key
            s = 1.0 - dist.get(truth, 0.0)
            m = mart.value if dup else mart.step(s)
            if not math.isfinite(m) or not (0.0 < mart.last_p <= 1.0):
                raise AssertionError(f"{q.question_id}: martingale {m} p {mart.last_p}")
            rec.update({"martingale": round(m, 3), "p_value": round(mart.last_p, 4), "dup": dup})
            if mart.fired and not dup:
                for b in beliefs:
                    b.discount(q.t_query + 1, a.factor)
                fires.append({"t": q.t_query, "day": q.day_index, "question_id": q.question_id, "martingale": round(m, 2)})
                mart.reset()
                rec["fired"] = True
        elif agent == "bma":
            wq = w_of(q.object_id)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--base", default="timetable", choices=("mostfreq", "timetable"), help="the counter the wrapper sits on")
    ...
    ap.add_argument("--detector", default="cusum", choices=("cusum", "mixture"), help="CUSUM e-detector (restarting) or the plain mixture martingale")
    ...
    ap.add_argument("--per-object", action="store_true", help="one fixed-share weight vector per object instead of one global vector")
